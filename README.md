...
Ожидаемые имена (пример **example.com**):

- `matrix.example.com` — Synapse (`MATRIX_DOMAIN`)
- `element.example.com` — Element Web (`ELEMENT_DOMAIN`)
- `admin.example.com` — `ADMIN_DOMAIN` (заглушка в Caddy, пока не подключите свой backend)
- `auth.example.com` — `AUTH_DOMAIN` (заглушка; под MAS/OIDC позже)
- `call.example.com` — Element Call (`CALL_DOMAIN`)
- `rtc.example.com` — LiveKit + JWT (`RTC_DOMAIN`)
- `turn.example.com` — TURN (`TURN_DOMAIN`), **coturn** в стеке по умолчанию (см. раздел TURN)
...
|------------|--------|------------|
| `MATRIX_DOMAIN` | `matrix.example.com` | Хост Synapse в Caddy, API `/_matrix`, `.well-known`, при стандартной установке = `server_name` и MXID `@user:matrix.chat.…` |
...
| `ELEMENT_DOMAIN` | `element.example.com` | Element Web — в браузере: `https://element.example.com/` |
| `CALL_DOMAIN` | `call.example.com` | Element Call (`element_call.url`) |
| `RTC_DOMAIN` | `rtc.example.com` | `/livekit/jwt`, `/livekit/sfu` |
| `ADMIN_DOMAIN` | `admin.example.com` | Заглушка 501 — при необходимости замените в Caddy |
| `AUTH_DOMAIN` | `auth.example.com` | Заглушка 501 — например MAS/OIDC позже |
...
Удобнее, когда **`MATRIX_DOMAIN` = хост API = `server_name` = суффикс MXID** (всё **`matrix.chat.…`**). В **`.env`** задайте **`MATRIX_DOMAIN=matrix.example.com`**, строку **`MATRIX_SERVER_NAME`** **не указывайте** (или оставьте равной `MATRIX_DOMAIN`) — тогда Element и **`caddy-init`** используют одно имя, отдельный DNS вида **`matrix.ptolemey.net`** не нужен.
...
- **Тестовый сервер, данные не жалко:** полный сбой БД и конфига Synapse, новый `server_name` с нуля:
  ...
  ```
  В **`.env`**: только **`MATRIX_DOMAIN=matrix.example.com`**, без второго домена; **`docker compose up -d`**. Заново создайте пользователей (**регистрация** или **`register_new_matrix_user`**). Затем **`docker compose --profile init run --rm element-init && docker compose up -d element lk-jwt caddy`**.
...
`curl -sS "https://matrix.example.com/_matrix/client/versions"`  
и с Origin:  
`curl -sSI -H "Origin: https://element.example.com" "https://matrix.example.com/_matrix/client/versions"`  
